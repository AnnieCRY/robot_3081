...
TEST_F(SensorTest, SensorCalculateReading) {
  ...
  lightsensor->calculateReading({60, 70}, 30);
  EXPECT_GT(lightsensor->get_left_reading(), 300)
   << "\n FAIL update left reading to the closer light";
  EXPECT_GT(lightsensor->get_right_reading(), 50)
   << "\n FAIL update right reading to the closer light";
